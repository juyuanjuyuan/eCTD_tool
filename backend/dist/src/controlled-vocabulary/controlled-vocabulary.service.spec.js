...
describe('ControlledVocabularyService', () => {
    ...
    beforeEach(async () => {
        ...
        service = module.get(controlled_vocabulary_service_1.ControlledVocabularyService);
    });
    ...
    describe('seedStfVocabularies', () => {
        ...
        it('should parse valid-values.xml and insert category + file-tag rows', async () => {
            ...
            await service.seedStfVocabularies();
            const createCalls = prisma.controlledVocabulary.create.mock.calls;
            expect(createCalls.length).toBeGreaterThan(0);
            const vocabNames = new Set(createCalls.map((c) => c[0].data.vocabularyName));
            ...
            const speciesRows = createCalls.filter((c) => c[0].data.vocabularyName === 'stf-category-species');
            ...
            const m4Rows = createCalls.filter((c) => c[0].data.vocabularyName === 'stf-file-tag-m4');
            const m5Rows = createCalls.filter((c) => c[0].data.vocabularyName === 'stf-file-tag-m5');
        ...
        it('should encode realm into descriptionEn prefix', async () => {
            ...
            await service.seedStfVocabularies();
            const createCalls = prisma.controlledVocabulary.create.mock.calls;
            const durationRow = createCalls.find((c) => c[0].data.vocabularyName === 'stf-category-duration' &&
                c[0].data.code === 'short');
    ...
    describe('getStfCategoryValues', () => {
        it('should return values for a single category', async () => {
            ...
            expect(cache.set).toHaveBeenCalledWith('cv:stf-category-values:species', expect.any(Array), 86400);
        });
    });
});
//# sourceMappingURL=controlled-vocabulary.service.spec.js.map