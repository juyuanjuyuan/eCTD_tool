...
describe('StudyTaggingFileService', () => {
    let service;
    beforeEach(() => {
        service = new study_tagging_file_service_1.StudyTaggingFileService();
    });
    describe('generateStfXml', () => {
        ...
        it('references the stf XSL stylesheet', () => {
            const xml = service.generateStfXml(buildStudy());
            expect(xml).toContain('<?xml-stylesheet type="text/xsl" href="../../../util/style/ich-stf-stylesheet-2-3.xsl"?>');
        });
        ...
        it('escapes XML entities in the study title', () => {
            const xml = service.generateStfXml(buildStudy({ title: 'A & B < C > "D"' }));
            expect(xml).toContain('<title>A &amp; B &lt; C &gt; &quot;D&quot;</title>');
        });
        it('emits <study-id> with the human-readable study number', () => {
            const xml = service.generateStfXml(buildStudy({ studyId: 'TOX-2024-042' }));
        ...
        it('emits xlink:href, xlink:type="simple", checksum, checksum-type="md5" on NEW documents', () => {
            ...
            expect(xml).toContain('checksum="5eb63bbbe01eeed093cb22bb8f5acdc3"');
        ...
        it('emits operation="delete" without xlink:href or checksum attributes', () => {
            ...
            const docContentBlock = xml.substring(xml.indexOf('<doc-content'), xml.indexOf('</doc-content>'));
        ...
        it('emits an inner <file-tag name=... info-type=.../> element for each doc-content', () => {
            const xml = service.generateStfXml(buildStudy());
            expect(xml).toContain('<file-tag name="study-report-body" info-type="ich"/>');
        });
    ...
    describe('parseStfXml', () => {
        it('extracts the study-id', () => {
            const xml = service.generateStfXml(buildStudy({ studyId: 'STUDY-XYZ' }));
            const parsed = service.parseStfXml(xml);
    ...
    describe('computeStfChecksum', () => {
        ...
        it('is stable across identical inputs', () => {
            const xml = service.generateStfXml(buildStudy());
            expect(service.computeStfChecksum(xml)).toBe(service.computeStfChecksum(xml));
        });
        it('produces different digests for different content', () => {
            const h1 = service.computeStfChecksum(service.generateStfXml(buildStudy({ studyId: 'A' })));
            const h2 = service.computeStfChecksum(service.generateStfXml(buildStudy({ studyId: 'B' })));
            expect(h1).not.toBe(h2);
        });
        it('matches the known MD5 of "hello world"', () => {
            expect(service.computeStfChecksum('hello world')).toBe('5eb63bbbe01eeed093cb22bb8f5acdc3');
        });
    });
    describe('validateStructure', () => {
        ...
        it('returns valid=false when two doc-content elements share an ID', () => {
            ...
            const result = service.validateStructure(xml);
            expect(result.valid).toBe(false);
            expect(result.errors.some((e) => /重复|duplicate|dup/i.test(e))).toBe(true);
        });
        ...
        it('reports malformed checksum', () => {
            const xml = service
                .generateStfXml(buildStudy())
                .replace('checksum="5eb63bbbe01eeed093cb22bb8f5acdc3"', 'checksum="not-a-real-md5"');
            const result = service.validateStructure(xml);
            expect(result.valid).toBe(false);
...
//# sourceMappingURL=study-tagging-file.service.spec.js.map