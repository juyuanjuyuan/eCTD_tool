...
function buildLeafNode(overrides = {}) {
    return {
        id: 'node1',
        isLeaf: true,
        operation: 'NEW',
...
describe('ValidatorService', () => {
    ...
    beforeEach(() => {
        ...
        service = new validator_service_1.ValidatorService(mockPrisma, md5Service);
        jest.clearAllMocks();
    ...
    describe('validate - Category 1: Basic Identification', () => {
        it('should produce INFO items for file count, size, and empty sections', async () => {
            ...
            const infoItems = result.items.filter((i) => i.severity === client_1.ValidationSeverity.INFO);
    ...
    describe('validate - Category 2: File Structure', () => {
        it('should flag files exceeding 500MB', async () => {
            ...
            expect(error.severity).toBe(client_1.ValidationSeverity.ERROR);
        });
        ...
        it('should pass for valid files', async () => {
            ...
            mockPrisma.sequence.findUnique.mockResolvedValue(seq);
            const result = await service.validate('seq1');
            const fileErrors = result.items.filter((i) => i.ruleCategory === '文件/文件夹' && i.severity === client_1.ValidationSeverity.ERROR);
    ...
    describe('validate - Category 3: ICH Backbone', () => {
        ...
        it('should allow all NEW operations on first sequence', async () => {
            ...
            mockPrisma.sequence.findUnique.mockResolvedValue(seq);
            const result = await service.validate('seq1');
            const backboneErrors = result.items.filter((i) => i.ruleCategory === 'ICH骨架文件' && i.severity === client_1.ValidationSeverity.ERROR);
    ...
    describe('validate - Category 4.2: Envelope Information', () => {
        ...
        it('should flag depend-apt-rat-sqt mismatch (4.2.10)', async () => {
            const seq = buildSequence({ sequenceNodes: [] });
            mockPrisma.sequence.findUnique.mockResolvedValue(seq);
            mockPrisma.cvDependency.findFirst.mockResolvedValue(null);
            const result = await service.validate('seq1');
        ...
        it('should pass with all valid envelope info', async () => {
            ...
            mockPrisma.sequence.findUnique.mockResolvedValue(seq);
            const result = await service.validate('seq1');
            const envelopeErrors = result.items.filter((i) => i.ruleCode.startsWith('4.2.') && i.severity === client_1.ValidationSeverity.ERROR);
    ...
    describe('validate - Category 4.3: Completeness', () => {
        it('should flag missing REQUIRED sections', async () => {
            ...
            const result = await service.validate('seq1');
            const error = result.items.find((i) => i.ruleCode === '4.3' && i.description.includes('必填章节缺失'));
            expect(error).toBeDefined();
        });
        it('should not flag completed REQUIRED sections', async () => {
            ...
            const result = await service.validate('seq1');
            const errors = result.items.filter((i) => i.ruleCode === '4.3' && i.description.includes('必填章节缺失'));
        ...
        it('should flag non-empty FORBIDDEN sections', async () => {
            ...
            const result = await service.validate('seq1');
            const warning = result.items.find((i) => i.ruleCode === '4.3' && i.description.includes('不应包含'));
            expect(warning).toBeDefined();
        });
        it('4.3.M5: should flag missing 5.3.5.1/5.3.5.2/5.3.5.3 when module 5 is present', async () => {
            ...
            expect(errors.every((e) => e.severity === client_1.ValidationSeverity.ERROR)).toBe(true);
    ...
    describe('validate - Category 6: PDF Analysis', () => {
        function buildPdfNode(pdfAnalysis) {
        ...
        it('should pass a fully compliant PDF', async () => {
            ...
            mockPrisma.sequence.findUnique.mockResolvedValue(seq);
            const result = await service.validate('seq1');
            const pdfErrors = result.items.filter((i) => i.ruleCategory === 'PDF分析' && i.severity === client_1.ValidationSeverity.ERROR);
    ...
    describe('getReport', () => {
        it('should query by report ID', async () => {
            ...
            expect(mockPrisma.validationReport.findUnique).toHaveBeenCalledWith(expect.objectContaining({ where: { id: 'r1' } }));
    ...
    describe('getLatestReport', () => {
        it('should query by sequence ID with latest first', async () => {
            ...
            expect(mockPrisma.validationReport.findFirst).toHaveBeenCalledWith(expect.objectContaining({
...
//# sourceMappingURL=validator.service.spec.js.map