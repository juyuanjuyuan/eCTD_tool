...
describe('EditLockService', () => {
    ...
    beforeEach(() => {
        ...
        service = new edit_lock_service_1.EditLockService(redis);
    });
    describe('acquireLock', () => {
        it('should acquire lock on unlocked node', async () => {
            ...
            expect(redis.setnx).toHaveBeenCalledWith('lock:node:node-1', expect.objectContaining({ userId: 'user-1' }), 1800);
        });
        ...
        it('should throw ConflictException when locked by another user', async () => {
            ...
            await expect(service.acquireLock('node-1', 'user-1', '张三')).rejects.toThrow(common_1.ConflictException);
        });
    });
    describe('releaseLock', () => {
        ...
        it('should throw when trying to release another user\'s lock', async () => {
            ...
            await expect(service.releaseLock('node-1', 'user-1')).rejects.toThrow(common_1.ForbiddenException);
        });
    });
    describe('queryLock', () => {
    ...
    describe('heartbeat', () => {
        it('should refresh TTL for lock holder', async () => {
            ...
            expect(result.userId).toBe('user-1');
            expect(redis.set).toHaveBeenCalledWith('lock:node:node-1', expect.objectContaining({ userId: 'user-1' }), 1800);
        });
...
//# sourceMappingURL=edit-lock.service.spec.js.map