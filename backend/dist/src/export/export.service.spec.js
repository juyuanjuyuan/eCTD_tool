...
describe('ExportService', () => {
    ...
    beforeEach(async () => {
        ...
        const module = await testing_1.Test.createTestingModule({
            providers: [
                ...
                { provide: word_export_service_1.WordExportService, useValue: wordExport },
                { provide: pdf_export_service_1.PDFExportService, useValue: pdfExport },
    ...
    describe('exportWordSingle', () => {
        it('should export Word document', async () => {
            ...
            expect(wordExport.exportDocument).toHaveBeenCalledWith(mockDocument.contentJson, expect.objectContaining({
                headerText: '2.3.S.1 一般性质',
            }));
        });
        it('should use custom headerText', async () => {
            await service.exportWordSingle('node-1', '自定义页眉');
            expect(wordExport.exportDocument).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
                headerText: '自定义页眉',
            }));
        });
        it('should throw when document content is empty', async () => {
            ...
            await expect(service.exportWordSingle('node-1')).rejects.toThrow(common_1.BadRequestException);
        });
        it('should throw when node not found', async () => {
            prisma.sequenceNode.findUnique.mockResolvedValue(null);
            await expect(service.exportWordSingle('nonexistent')).rejects.toThrow(common_1.NotFoundException);
        });
        it('should throw when document not found', async () => {
            prisma.document.findUnique.mockResolvedValue(null);
            await expect(service.exportWordSingle('node-1')).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('exportPdfSingle', () => {
        ...
        it('should throw when contentHtml is empty', async () => {
            ...
            await expect(service.exportPdfSingle('node-1')).rejects.toThrow(common_1.BadRequestException);
        });
    ...
    describe('getTaskStatus', () => {
        ...
        it('should throw when job not found', async () => {
            ...
            await expect(service.getTaskStatus('nonexistent')).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('getDownloadUrl', () => {
        it('should throw when job not completed', async () => {
            ...
            await expect(service.getDownloadUrl('job-1')).rejects.toThrow(common_1.BadRequestException);
        });
        it('should throw when no download object name', async () => {
            ...
            await expect(service.getDownloadUrl('job-1')).rejects.toThrow(common_1.NotFoundException);
        });
        it('should throw when job not found', async () => {
            exportQueue.getJob.mockResolvedValue(null);
            await expect(service.getDownloadUrl('nonexistent')).rejects.toThrow(common_1.NotFoundException);
        });
    });
