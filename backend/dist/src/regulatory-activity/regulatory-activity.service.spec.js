...
describe('RegulatoryActivityService', () => {
    ...
    beforeEach(async () => {
        ...
        const module = await testing_1.Test.createTestingModule({
            providers: [
                regulatory_activity_service_1.RegulatoryActivityService,
                { provide: prisma_service_1.PrismaService, useValue: prisma },
                { provide: controlled_vocabulary_service_1.ControlledVocabularyService, useValue: cvService },
            ],
        }).compile();
        service = module.get(regulatory_activity_service_1.RegulatoryActivityService);
    });
    describe('create', () => {
        ...
        it('should throw when application not found', async () => {
            ...
            await expect(service.create('x', { regulatoryActivityTypeCode: 'cnrat1' })).rejects.toThrow(common_1.NotFoundException);
        });
        it('should throw when dependency is invalid', async () => {
            cvService.validateDependency.mockResolvedValue(false);
            await expect(service.create('app-1', { regulatoryActivityTypeCode: 'cnrat9' })).rejects.toThrow(common_1.BadRequestException);
        });
    });
    describe('findAllByApplication', () => {
        it('should return all regulatory activities', async () => {
            ...
            expect(prisma.regulatoryActivity.findMany).toHaveBeenCalledWith(expect.objectContaining({
...
//# sourceMappingURL=regulatory-activity.service.spec.js.map