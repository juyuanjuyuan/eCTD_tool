...
describe('ApprovalService', () => {
  ...
  describe('submitForApproval', () => {
    ...
    it('should throw when node not found', async () => {
      ...
      await expect(service.submitForApproval('x', 'u')).rejects.toThrow(NotFoundException);
    });

    it('should throw when node is not a leaf', async () => {
      prisma.sequenceNode.findUnique.mockResolvedValue({ ...mockLeafNode, isLeaf: false });

      await expect(service.submitForApproval('node-1', 'u')).rejects.toThrow(BadRequestException);
    });
  ...
  describe('approveNode', () => {
    ...
    it('should throw when not SUBMITTED', async () => {
      prisma.sequenceNode.findUnique.mockResolvedValue(mockLeafNode); // DRAFT

      await expect(service.approveNode('node-1', 'a')).rejects.toThrow(BadRequestException);
    });

    it('should throw when node not found', async () => {
      prisma.sequenceNode.findUnique.mockResolvedValue(null);

      await expect(service.approveNode('x', 'a')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('rejectNode', () => {
    ...
    it('should throw when not SUBMITTED', async () => {
      ...
      await expect(service.rejectNode('node-1', 'a', 'r')).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('getApprovalHistory', () => {
    ...
    it('should throw when node not found', async () => {
      ...
      await expect(service.getApprovalHistory('x')).rejects.toThrow(NotFoundException);
    });
  });
