...
describe('CommentService', () => {
  ...
  describe('createComment', () => {
    ...
    it('should throw when node not found', async () => {
      ...
      await expect(
        service.createComment('x', 'user-1', { content: 'test' }),
    ...
    it('should throw when parent comment not found', async () => {
      ...
      await expect(
        service.createComment('node-1', 'user-1', { content: 'reply', parentId: 'x' }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw when parent belongs to different node', async () => {
      ...
      await expect(
        service.createComment('node-1', 'user-1', { content: 'reply', parentId: 'parent-1' }),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('deleteComment', () => {
    ...
    it('should throw when non-author USER tries to delete', async () => {
      await expect(
        service.deleteComment('comment-1', 'user-2', 'USER'),
    ...
    it('should throw when comment not found', async () => {
      ...
      await expect(
        service.deleteComment('x', 'user-1', 'USER'),
      ).rejects.toThrow(NotFoundException);
    });
  });
