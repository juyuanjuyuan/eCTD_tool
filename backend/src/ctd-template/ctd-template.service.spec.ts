...
describe('CtdTemplateService', () => {
  let service: CtdTemplateService;
  ...
  beforeEach(async () => {
    ...
    service = module.get<CtdTemplateService>(CtdTemplateService);
  });
  ...
  describe('getTemplateTree', () => {
    it('should build tree from flat query', async () => {
      ...
      expect(result[0].children).toHaveLength(1); // t-23s
      expect(result[0].children[0].children).toHaveLength(1); // t-23s1
    });
  ...
  describe('initializeSequenceNodes', () => {
    ...
    it('should set operation=NEW for leaf nodes on first sequence', async () => {
      ...
      // Each call is a promise from prisma.sequenceNode.create
      // We check the create calls
      const createCalls = prisma.sequenceNode.create.mock.calls;
      const leafCreates = createCalls.filter((c: any) => c[0].data.isLeaf === true);
  ...
  describe('updateSequenceNode', () => {
    it('should update node properties', async () => {
      ...
      const result = await service.updateSequenceNode('seq-1', 'n-1', { status: 'COMPLETED' as any });
  ...
  describe('updateBackboneAttributes', () => {
    ...
    it('should throw for unsupported section', async () => {
      ...
      await expect(
        service.updateBackboneAttributes('seq-1', 'n-3', { substance: 'test' }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('should mark child leaves for rebuild when substance changes on non-first sequence', async () => {
      ...
      prisma.sequenceNode.findMany
        .mockResolvedValueOnce([ // isLeaf query
          { id: 'leaf-1', isLeaf: true, parentId: 'n-s', templateNodeId: 't-1' },
        ])
        .mockResolvedValueOnce([ // all nodes query
      ...
      // Should update child leaf to NEW
      expect(prisma.sequenceNode.update).toHaveBeenCalledWith({
        where: { id: 'leaf-1' },
        data: { operation: 'NEW' },
      });
  ...
  describe('createExtensionNode', () => {
    ...
    it('should reject for non-biological products (cnprt1)', async () => {
      ...
      await expect(
        service.createExtensionNode('seq-1', 'n-32r', { extensionType: '3.2.R.1' }),
    ...
    it('should reject for non-extension parent node', async () => {
      ...
      await expect(
        service.createExtensionNode('seq-1', 'n-1', { extensionType: '3.2.R.1' }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('should reject invalid extension type', async () => {
      ...
      await expect(
        service.createExtensionNode('seq-1', 'n-32r', { extensionType: '3.2.R.99' }),
      ).rejects.toThrow(BadRequestException);
    });
  });
