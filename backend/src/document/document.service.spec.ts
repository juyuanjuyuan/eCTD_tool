...
describe('DocumentService', () => {
  ...
  describe('saveDocument', () => {
    ...
    it('should throw ForbiddenException when node is APPROVED', async () => {
      ...
      await expect(service.saveDocument('node-1', saveDto)).rejects.toThrow(ForbiddenException);
    });

    it('should throw NotFoundException when node does not exist', async () => {
      prisma.sequenceNode.findUnique.mockResolvedValue(null);

      await expect(service.saveDocument('nonexistent', saveDto)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should count Chinese characters and English words separately', async () => {
      ...
      // "中文字符测试" = 6 Chinese chars (中文字符测试), "and English words here" = 4 English words
  ...
  describe('getVersion', () => {
    ...
    it('should throw NotFoundException when document not found', async () => {
      ...
      await expect(service.getVersion('node-1', 1)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw NotFoundException when version not found', async () => {
      ...
      await expect(service.getVersion('node-1', 99)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('createVersionSnapshot', () => {
    ...
    it('should throw NotFoundException when document not found', async () => {
      ...
      await expect(service.createVersionSnapshot('node-1')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('restoreVersion', () => {
    ...
    it('should save current state and restore old version', async () => {
      ...
      // Should save current state as snapshot first
      expect(prisma.documentVersion.create).toHaveBeenCalledWith({
        data: expect.objectContaining({
          documentId: 'doc-1',
          version: 3,
      ...
      // Should restore with old version content
      expect(prisma.document.update).toHaveBeenCalledWith({
        where: { nodeId: 'node-1' },
        data: expect.objectContaining({
          contentHtml: '<p>old content</p>',
    ...
    it('should throw NotFoundException when version not found', async () => {
      ...
      await expect(service.restoreVersion('node-1', 99)).rejects.toThrow(NotFoundException);
    });
  });
});
