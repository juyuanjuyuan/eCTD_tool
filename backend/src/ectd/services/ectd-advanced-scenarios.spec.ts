/**
 * eCTD Advanced Scenario Tests — Plan 8 Stage 1.3 + 1.4
 *
 * Lifecycle Scenarios:
 * - Scenario 4: Withdraw sequence (4-step process)
 * - Scenario 5: Format conversion sequence (cnsqt4)
 * - Scenario 6: File reuse across sequences
 * - Scenario 7: Backbone attribute update (delete old, rebuild)
 *
 * Boundary Cases:
 * - xml:lang handling for foreign reference materials
 * - Extension nodes (bioproduct 3.2.R only)
 * - Empty section filtering
 * - Large number of leaf elements (200+ files)
 */
...
describe('eCTD Advanced Scenarios', () => {
  ...
  // ==============================================================
  // 1.3 Scenario 4: Withdraw Sequence (4-step process)
  // ==============================================================
  ...
  describe('Lifecycle: Withdraw sequence (cnsqt3)', () => {
    ...
    beforeEach(() => {
      lifecycleService = new LifecycleService(lifecyclePrisma as any);
      jest.clearAllMocks();
    });
    ...
    it('should generate correct 4-step withdraw operations for mixed sequence', async () => {
      lifecyclePrisma.sequenceNode.findMany.mockResolvedValue([
        ...
        { templateNodeId: 'tpl4', operation: LeafOperation.APPEND, isLeaf: true },
      ]);
    ...
    it('should validate withdraw sequence type code via validator', async () => {
      const seq = buildSequence({
        sequenceTypeCode: 'cnsqt3', // withdraw type
        sequenceNodes: [
      ...
      // cnsqt3 is a valid sequence type, should not error
  ...
  // ==============================================================
  // 1.3 Scenario 5: Format Conversion Sequence (cnsqt4)
  // ==============================================================
  ...
  describe('Lifecycle: Format conversion sequence (cnsqt4)', () => {
    ...
    it('should allow REPLACE operations in format conversion sequence', async () => {
      ...
      // Should not have lifecycle errors for valid REPLACE
      const lifecycleErrors = result.items.filter(
  ...
  // ==============================================================
  // 1.3 Scenario 6: File Reuse (cross-sequence reference)
  // ==============================================================
  ...
  describe('Lifecycle: File reuse across sequences', () => {
    it('should accept referenced files with valid paths', async () => {
            ...
            fileAttachments: [
              {
                ...
                isReference: true, // referenced from prior sequence
                pdfAnalysis: {
      ...
      // No file naming errors for valid referenced files
      const namingErrors = result.items.filter(
    ...
    it('should validate referenced file paths are compliant', async () => {
      ...
      // Even referenced files should follow naming rules
  ...
  // ==============================================================
  // 1.3 Scenario 7: Backbone attribute update
  // ==============================================================
  ...
  describe('Lifecycle: Backbone attribute update (3.2.S/3.2.P rebuild)', () => {
    it('should validate NEW operations for rebuilt sections after attribute change', async () => {
      // When substance/manufacturer changes, child leaves should be NEW (rebuild)
      const seq = buildSequence({
        sequenceNumber: '0001',
        sequenceNodes: [
          buildLeaf({
      ...
      // NEW operation should pass (even on non-0000 sequence when rebuilding)
      const deleteOrRebuildErrors = result.items.filter(
        i => i.ruleCode === '3.10' && i.description?.includes('3.2.S.1'),
      );
  ...
  // ==============================================================
  // 1.4 Boundary: xml:lang for foreign reference materials
  // ==============================================================
  ...
  describe('Boundary: xml:lang handling', () => {
    it('should accept zh (Chinese) as valid xml:lang', async () => {
      ...
      // Should have no language-related errors
      const langErrors = result.items.filter(
        i => i.severity === ValidationSeverity.ERROR && i.description?.includes('lang'),
      );
  ...
  // ==============================================================
  // 1.4 Boundary: Extension nodes (bioproduct 3.2.R)
  // ==============================================================
  ...
  describe('Boundary: Extension nodes (3.2.R)', () => {
    it('should generate node-extension elements in index.xml for bioproduct', async () => {
      ...
      expect(xml).toContain('</node-extension>');
      // Leaves are rendered as <leaf> tags, not by elementName
    ...
    it('should validate extension node with correct product type (cnprt2 bioproduct)', async () => {
      const seq = buildSequence({
        regulatoryActivity: {
          application: { productTypeCode: 'cnprt2' }, // bioproduct
        },
        sequenceNodes: [
          buildLeaf({
      ...
      // cnprt2 (bioproduct) should allow extension nodes
  ...
  // ==============================================================
  // 1.4 Boundary: Empty section filtering
  // ==============================================================
  ...
  describe('Boundary: Empty section filtering', () => {
    ...
    it('should not generate XML for cn-regional empty sections', async () => {
      ...
      const md5 = new Md5Service();
      const cnService = new CnRegionalXmlService(cnPrisma as any, md5);
    ...
    it('should include section when at least one child has active leaf', async () => {
      ...
      // Parent section should appear because one child is active
      expect(xml).toContain('m2-summaries');
      // Active leaf rendered as <leaf> with its title and file path
      expect(xml).toContain('<title>有内容章节</title>');
      expect(xml).toContain('m2/23-qos/doc.pdf');
      // Empty leaf should not appear
      expect(xml).not.toContain('空章节');
    });
  });

  // ==============================================================
  // 1.4 Boundary: Large number of leaf elements (200+)
  // ==============================================================
  ...
  describe('Boundary: Large number of leaf elements (200+ files)', () => {
    it('should generate valid index.xml with 200+ leaves', async () => {
      ...
      // Should contain parent section and leaf content via titles/paths
      ...
      // Verify all 220 leaves are present
    ...
    it('should validate sequence with 200+ leaves without errors on valid data', async () => {
      ...
      // No errors for valid data — all naming is compliant, all PDFs valid
