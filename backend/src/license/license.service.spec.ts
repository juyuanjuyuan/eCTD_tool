...
describe('LicenseService', () => {
  ...
  describe('getMachineId', () => {
    ...
    it('honors LICENSE_MACHINE_ID_OVERRIDE as fallback', () => {
      ...
      // recreate service so cached id is empty
      const fresh = new LicenseService(prisma as any);
  ...
  describe('verifyLicenseString', () => {
    it('rejects malformed input', () => {
      ...
      expect(service.verifyLicenseString('a.b').valid).toBe(false); // bad base64url
    });
  ...
  describe('activate', () => {
    ...
    it('rejects an invalid license without persisting', async () => {
      await expect(service.activate('garbage')).rejects.toBeInstanceOf(BadRequestException);
    ...
    it('upsert is idempotent on same code', async () => {
      ...
      // upsert with `where: { code }` is idempotent at SQL level
