...
| H11 | 2026-04-26 | 上传 TXT 可立即进表；上传 PDF toast/overlay 后"已上传文件"仍是 Empty，切到别的模块再回来才出现 | PDF 路径会跑 `FileService.analyzePdf()` 并写 `file_pdf_analysis`，TXT 不会。SQLite 桌面版把 PG 的 `Json? complianceDetails` 降级为 `String?`，但写入侧仍传 `{errors,warnings}` object，Prisma 抛 `Expected String or Null, provided Object`。attachment 和本地文件已先创建，analysis 写库失败使 upload API 500，前端拿不到 `newFile`，optimistic insert 不执行；重新进入节点时 listFiles 又能列出已创建 attachment | `db110ef9` | (1) `backend/src/file/file.service.ts:746` `analyzePdf()` 构造 `complianceDetails` 后按 `this.prisma.dbProvider === 'sqlite'` 写 JSON string，否则写 JSON object；(2) `backend/src/file/file.service.spec.ts:138` 增 SQLite 写入回归测试；(3) `desktop/main/index.ts:139` 增 `--open-devtools` / `ECTD_OPEN_DEVTOOLS=1` 生产调试开关 | (1) SQLite-as-string 字段必须同时覆盖写入序列化和读取反序列化，不能只修 serializer 出口；(2) 所有 PDF 上传回归必须看 Network upload status，toast 成功不是证据；(3) 保留生产 DevTools 开关，现场调试必须先抓 Console/Network 再改代码；(4) 冷装验收记录：PDF upload 201，Empty→`已上传文件 (1)`，切回仍在，Console 0 应用红字 |
| ... | | | | | | |
