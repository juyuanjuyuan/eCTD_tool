...
- [x] 每种注册行为类型下支持的序列类型联动
- [x] 前端创建序列时，选择申请类型后级联过滤可选的注册行为类型和序列类型

### 0.3 eCTD 编号规则确认
- [x] **原始编号**: 10 位数字 = 4 位年份 + 6 位流水号（如 2026123456）
- [x] **申请编号**: 字母前缀 + 4 位年份 + 5 位流水号（如 x202612345）
  - 前缀规则: x=化学药品临床/新药/仿制药, y=生物制品, l=进口药品, s=原料药
  - 申请编号在创建后不可更改（跨序列不可变）
- [x] **序列号**: 4 位数字，从 0000 开始递增，同一注册行为内连续（不允许跳号）
- [x] 编号唯一性验证（全局唯一）

### 0.4 信封元素规则确认（cn-regional.xml 的 cn-envelope）
- [x] 信封包含 3 个层级共 12 个属性，**所有属性均为必填**:
...
- [x] 受控词汇属性值必须使用代码名称（如 cnapt2），不使用显示值

### 0.5 角色权限矩阵
- [x] ADMIN: 用户管理、项目管理
- [x] MANAGER: 项目管理、审批、强制解锁
- [x] EDITOR: 文档编辑、文件上传
- [x] VIEWER: 只读
...
### 1.1 NestJS 项目初始化
- [x] 创建 NestJS 项目 (`nest new backend`)
...
- [x] 执行首次迁移（migration `20260321034135_init` 已成功创建全部 8 张表）
- [x] 创建 PrismaModule / PrismaService

### 1.3 受控词汇服务
- [x] 创建 `ControlledVocabularyModule`
- [x] XML 解析服务: 启动时自动解析所有 cv-*.xml 和 depend-apt-rat-sqt.xml
- [x] GET `/api/v1/cv/application-types` — 获取申请类型列表
...
- [x] 分页 DTO (PaginationDto)
- [x] 参数校验管道 (ValidationPipe)

## 阶段 2: 认证模块

### 2.1 后端
- [x] AuthModule: 注册、登录、Token 刷新
- [x] JwtStrategy + JwtAuthGuard
- [x] RolesGuard + @Roles() 装饰器
- [x] UserModule: 用户 CRUD (ADMIN 管理)
- [x] 密码 bcrypt 加密

### 2.2 前端
- [x] React + Vite 项目初始化
- [x] Ant Design Pro 布局集成
- [x] 登录页面
- [x] 路由守卫 (ProtectedRoute)
- [x] Axios 拦截器 (Token 管理)

## 阶段 3: 项目管理

### 3.1 后端
- [x] ProjectModule: 创建/列表/详情/更新/归档
- [x] 项目成员管理 (添加/移除成员)
- [x] 按当前用户的项目成员身份过滤项目列表
- [x] 原始编号生成服务（4位年份 + 6位流水号，全局唯一校验）

### 3.2 前端
- [x] 项目列表页 (ProTable)
- [x] 创建项目弹窗
- [x] 项目详情页（Tab 页: 概览/申请/成员）

## 阶段 4: 申请与序列管理（含信封元素验证）

### 4.1 申请管理后端
- [x] ApplicationModule: 创建/列表/详情/更新
- [x] 创建申请时:
  - 必填: 申请类型(受控词汇)、产品类型(受控词汇)、原始编号
  - 自动生成申请编号（前缀根据产品类型和申请类型决定）
  - 验证原始编号格式（10位数字）
  - 验证申请编号唯一性
- [x] **申请创建后，申请级别信封属性不可修改**（application-id, application-type, product-type, product-number）

### 4.2 注册行为管理后端
- [x] RegulatoryActivityModule: 创建/列表
- [x] 创建注册行为时:
  - 验证 regulatory-activity-type 在当前申请类型下是否允许（依据 depend-apt-rat-sqt.xml）
  - 设置 related-sequence（关联上一个序列号，首次为 0000）
- [x] **同一注册行为内，注册行为级别信封属性不可修改**

### 4.3 序列管理后端
- [x] SequenceModule: 创建/列表/详情/更新
- [x] 创建序列时:
...
- [x] 序列状态: DRAFT → EDITING → VALIDATING → EXPORTED → SUBMITTED
...
- [x] 受控词汇正确解析并可通过 API 查询
- [x] 申请类型→注册行为类型→序列类型级联选择正确
- [x] 申请编号按规则自动生成，前缀正确
- [x] 序列号从 0000 自动递增，不允许跳号
- [x] 信封元素不可变性约束正确执行（申请级别创建后不可改、注册行为级别同一活动内不可改）
- [x] 用户可注册/登录，权限控制正常
- [x] Docker 一键启动开发环境（已验证 `docker-compose up -d` 启动 PostgreSQL + Redis + MinIO 正常）
