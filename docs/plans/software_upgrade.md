...
- **H7（2026-04-26）** 桌面"创建序列"弹窗中"注册行为类型"下拉为空（Web 版正常 9 项）。**根因**：受控词汇三道防线全部 silent fail——构建期 `build-firstrun-db.js seedControlledVocabularies()` 找不到 reference 时 `warn + return`（默默产出残缺 first-run.db）；STF 段甚至根本没真 seed，留一句 log "will be seeded at runtime"；运行期 `data-dir.ts` reference 释放失败只 `log.info` 不报错；运行期 `controlled-vocabulary.service.ts` 用 `count() > 0` 全表短路自愈不了 partial state，且 STF 路径用 `process.cwd()` 而非 REFERENCE_DIR env，bundled 模式必拼错路径。**修复（6 处协同）**：①`controlled-vocabulary.service.ts` 改 per-vocabulary `count(where vocabularyName=X)` 自愈，cnrat/cnapt/cnprt/cnsqt/cv_dependency 各自独立检查；STF 路径改 `REFERENCE_DIR` env 派生；找不到 reference 从 warn 升级 error 并标注修复线索。②`build-firstrun-db.js` 所有 silent skip 改 throw；新增 `getReferenceXmlDir()` 支持 `REFERENCE_DIR` env 覆盖；STF 段改成构建期真 seed（端口 runtime service 的 `parseStfValidValuesFile` 解析逻辑——root 是 `<ectd:study-values>`、值在 `@_value` 属性、version 硬编 `6.0`/validFrom 硬编 `2023-11-01`）；新增 cv_dependency 表构建期 seed；末尾 hard assertion `cnapt=4 cnprt=2 cnrat=9 cnsqt=4 stfCat>=1 stfTag>=1 ctdNodes>=200 ctdRules>=100`，少一项 throw。③`scripts/build-electron.sh [4/6]` rsync 后加 6 个 XML 存在性硬校验（4 个 cv-* + depend-apt-rat-sqt + STF valid-values）；缺一即 exit 1。④`scripts/postbuild-verify.sh` `required[]` 追加 6 个 reference XML 路径；新增 sqlite3 内容断言：从 first-run.db 反查行数，`cnrat != 9` 直接构建红。⑤`desktop/main/data-dir.ts` reference 释放失败 `log.info` → `log.error`，error 文案含可操作排查线索。⑥沙箱冒烟：`SKIP_TSC=true node scripts/build-firstrun-db.js` 跑通，counts: `users=1 ctdNodes=229 rules=196 cv=238 (apt=4 prt=2 rat=9 sqt=4) cvDep=75 stfCat=25 stfTag=194`；`REFERENCE_DIR=/tmp/nonexistent` → 立刻 throw。**真机验证**：你需要 rebuild dmg 装一次，确认创建序列时下拉有 9 项 cnrat 选项。
...
- **H11（commit db110ef9, 2026-04-26）** PDF 上传后前端仍显示 Empty，但 TXT 上传正常，切节点回来 PDF 又出现。**证据**：生产 DMG 用 `--open-devtools --remote-debugging-port=9222` 跑真实前端上传，PDF 的 `POST /api/v1/nodes/:nodeId/files/upload` 返回 500；main.log 报 `filePdfAnalysis.create()` 参数错误：`complianceDetails` 期望 `String or Null` 却收到 Object。**根因**：SQLite schema 把 `FilePdfAnalysis.complianceDetails` 从 PG 的 `Json?` 映射为 `String?`，H9/H10 只补了读取侧 parse 和响应 shape，PDF 写入侧 `analyzePdf()` 仍把 `{errors,warnings}` object 直接写进 SQLite 字符串列。附件记录和本地文件已在 analysis 前创建，所以 API 500 时前端拿不到 upload 返回，切节点回来 listFiles 又能看到 attachment。**修复**：`FileService.analyzePdf()` 根据 `PrismaService.dbProvider` 分支，SQLite 写 JSON string，PG 继续写 JSON object；补 SQLite 写入回归测试；Electron main 增加 `--open-devtools` / `ECTD_OPEN_DEVTOOLS=1` 可控调试入口。**冷装验收**：最终 DMG SHA256 `967fd3662abce529d8880c1ca6932aac42689b23564014e565527eaa2da3b110`；冷装后真实 UI 上传 PDF 返回 201，2 秒内文件表从 Empty 变为 `已上传文件 (1)`，离开页面再回同节点仍 1 行，DevTools Console 0 应用红字。
